...
export async function PATCH(
  ...
  try {
    const guide = await prisma.$transaction(async (tx) => {
      const updatedGuide = await tx.guideEntry.update({
        where: { id },
        data: {
          editorialStatus,
      ...
      await tx.guideReviewLog.create({
        data: {
          guideId: id,
    ...
    console.error('[Admin Guides API] PATCH error:', error);
    return NextResponse.json(
      { error: getApiLocalizedText(req, 'Failed to update guide review status.') },
      { status: 500 }
    );
  }
}
