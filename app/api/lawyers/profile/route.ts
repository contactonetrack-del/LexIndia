...
export async function GET() {
  ...
  if (session.user.role !== 'LAWYER') {
    return NextResponse.json({ error: 'Only lawyers can access this endpoint.' }, { status: 403 });
  }
...
export async function PATCH(req: NextRequest) {
  ...
  if (session.user.role !== 'LAWYER') {
    return NextResponse.json({ error: 'Only lawyers can update their profile.' }, { status: 403 });
  }
  ...
  if (bio !== undefined && (typeof bio !== 'string' || bio.length > 2000)) {
    return NextResponse.json({ error: 'bio must be a string under 2000 characters.' }, { status: 400 });
  }
  if (city !== undefined && (typeof city !== 'string' || city.length > 100)) {
    return NextResponse.json({ error: 'city must be a string under 100 characters.' }, { status: 400 });
  }
  if (state !== undefined && (typeof state !== 'string' || state.length > 100)) {
    return NextResponse.json({ error: 'state must be a string under 100 characters.' }, { status: 400 });
  }
  if (experienceYears !== undefined && (typeof experienceYears !== 'number' || experienceYears < 0 || experienceYears > 60)) {
    return NextResponse.json({ error: 'experienceYears must be a number between 0 and 60.' }, { status: 400 });
  }
  if (consultationFee !== undefined && (typeof consultationFee !== 'number' || consultationFee < 0)) {
    return NextResponse.json({ error: 'consultationFee must be a non-negative number.' }, { status: 400 });
  }
  ...
  if (consultationModes !== undefined && (!Array.isArray(consultationModes) || consultationModes.some((m) => !validModes.includes(m as string)))) {
    return NextResponse.json({ error: 'consultationModes must be an array of VIDEO, CALL, CHAT.' }, { status: 400 });
  }
